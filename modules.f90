...
500     if (L< lmax_extrap_highl) &
    ...
    public thermo,inithermo,vis,opac,expmmu,dvis,dopac,ddvis,lenswin, tight_tau,& !IonizationFunctionsAtTime
...
!        vis, dvis, ddvis, expmmu, lenswin)
    real(dl) :: tau
!    real(dl), dimension(:), allocatable, intent(out):: opac, dopac, vis, dvis, ddvis, expmmu, lenswin
    real (dl):: ddopac
    real(dl) d
    integer i, j2
